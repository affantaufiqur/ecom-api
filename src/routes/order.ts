...
app.post("/order", authMiddleware, async (req, res) => {
  try {
    ...
    if (!checkOrderByCartId) {
      ...
      //TODO: also put qty and price here
      return res.json({ message: "Orders placed" });
    }

    const cart = await db.select().from(cart_items).where(eq(cart_items.cart_id, cart_id));
    if (cart.length === 0) return res.status(404).json({ message: "Cant make orders, because cart is empty" });
